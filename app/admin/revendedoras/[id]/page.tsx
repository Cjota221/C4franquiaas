...
export default function AdminProdutosPage() {
  ...
  return (
    <div className="p-6">
      <button onClick={() => router.push('/admin/revendedoras')} className="flex items-center gap-2 mb-4"><ArrowLeft size={20} />Voltar</button>
      <h1 className="text-2xl font-bold mb-4">Produtos - {revendedora?.store_name}</h1>
      <div className="grid grid-cols-3 gap-4">
        {produtos.map(p => {
          const v = vinculados.get(p.id);
          const ativo = v?.ativo || false;
          return (
            <div key={p.id} className={g-white rounded-lg shadow border-2  Arquivo criado{ativo ? 'border-green-500' : 'border-gray-200'}}>
              <div className="relative h-48 bg-gray-100"><Image src={p.imagem_principal || ''} alt={p.nome} fill className="object-cover" sizes="33vw" /></div>
              <div className="p-4">
                <h3 className="font-semibold mb-2">{p.nome}</h3>
                <p className="text-sm mb-2">R$ {p.preco.toFixed(2)}</p>
                <button onClick={() => toggle(p.id)} className={w-full py-2 rounded-lg  text-white}>{ativo ? 'Desativar' : v ? 'Ativar' : 'Vincular'}</button>
              </div>
            </div>
          );
        })}
