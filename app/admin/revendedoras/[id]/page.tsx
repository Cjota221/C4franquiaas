...
interface Revendedora { id: string; store_name: string; name: string; email: string; }
interface Produto { id: string; nome: string; preco: number; imagem_principal: string | null; }
interface ProdutoVinculado { produto_id: string; margem_lucro: number; ativo: boolean; }
...
export default function AdminProdutosPage() {
  ...
  const [vinculados, setVinculados] = useState<Map<string, ProdutoVinculado>>(new Map());
  const [loading, setLoading] = useState(true);

  const carregarDados = useCallback(async () => {
    const { data: r } = await supabase.from("resellers").select("*").eq("id", id).single();
    setRevendedora(r as Revendedora | null);
    ...
    const map = new Map<string, ProdutoVinculado>();
    (v as ProdutoVinculado[])?.forEach(x => map.set(x.produto_id, x));
    setVinculados(map);
    setLoading(false);
  }, [id, supabase]);
