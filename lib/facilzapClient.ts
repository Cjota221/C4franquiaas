...
function extractImageUrl(x: unknown): string | undefined {
  ...
  return undefined;
      ...
      // estoque: prefer sum of variation-level estoque when present (products often use 'variacao' rules)
      let estoque = 0;
      if (Array.isArray(p.variacoes) && p.variacoes.length > 0) {
        // sum individual variation estoque values if available
        estoque = p.variacoes.reduce((acc, v) => {
          if (!v) return acc;
          const ve = (v as any).estoque;
          if (typeof ve === 'number') return acc + ve;
          if (ve && typeof ve === 'object' && typeof ve['estoque'] === 'number') return acc + (ve['estoque'] as number);
          return acc;
        }, 0);
      }
      ...
      // fallback to raw preco if still null
        ...
        if (typeof p.preco === 'string') {
          ...
          export async function fetchAllProdutosFacilZap(): Promise<{ produtos: ProdutoDB[]; pages: number }> {
            ...
            while (true) {
              ...
              pagesConsumed++;
              // debug logging when requested to help diagnose missing products/fields
